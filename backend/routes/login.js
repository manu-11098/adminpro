...
app.post('/google', async (req, res, next) => {
	...
	try {
		...
		User.findOne({ email: googleUser.email }).exec()
        .then(user => {
            if (!user) {
                let user = new User();
        ...
        .then(user => {
			if (user.google) {
				const token = jwt.sign({ data: user }, SEED, { expiresIn: 14400 });
				res.status(200).json({ ok: true, user: user, token: token });
			} else {
				res.status(401).json( { ok: false, message: `User with email: ${user.email} can not login by google`})
			}
        }).catch(err => {
            res.status(500).json({ ok: false,  message: 'Error al buscar usuarios', errors: err.message });
        });
...
app.post('/', async (req, res) => {
	try {
		...
		res.status(500).json({ ok: false, message: 'Error al buscar usuarios', error } );
	}
